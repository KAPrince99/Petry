...
export function BoardView({ board }: BoardViewProps) {
  ...
  const { mutate: saveBoard, isPending } = useMutation({
    mutationFn: (patch: {
      title: string;
    ...
    meta: {
      errorMessage: "Could not save board changes.",
      toastId: "update-board",
    },
    onSuccess: async () => {
      await invalidateBoardQueries();
      closeEdit();
      toastSuccess("Board updated", "update-board-success");
    },
  });

  const { mutate: createTask, isPending: isTaskPending } = useMutation({
    ...
    onSuccess: async () => {
      ...
      closeCreateTask();
      toastSuccess("Task created", "create-task-success");
    },
  });
  ...
  const handleUpdateBoard = useCallback(
    ...
    [editColor, editDescription, editTitle, saveBoard],
  );
